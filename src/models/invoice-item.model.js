...
exports.createInvoiceLine = async (conn, order_id, product_id, qty, finalPrice, line_total) => {
  try {
    const [result] = await conn.query(`INSERT INTO invoice_item (invoice_id, product_id, quantity, unit_price, line_total)
             VALUES (?, ?, ?, ?, ?)`, [order_id, product_id, qty, finalPrice, line_total]);
    return {
      success: true,
